...
# @dataclasses.dataclass(config=dataclasses.PyDanticConfigForAnyObject)
class Trainer:
    def __init__(self, model: GaussianModelOriginBase, cfg: config_def.Config, scene_extent: float) -> None:
        ...
        self.strategy = GaussianStrategyOrigin(**(dataclasses.asdict(cfg.train.strategy)))
...
def __main():
    ...
    # init_original_3dgs_model(model, ds.dataset_point_cloud.xyz, ds.dataset_point_cloud.rgb)
    ...
    # breakpoint()
    trainer = Trainer(model, cfg, ds.extent)
