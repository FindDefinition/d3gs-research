...
class ColmapPoint3d(HomogeneousTensor):
    ...
    num_image_touch: Annotated[torch.Tensor, arrcheck.ArrayCheck(["N"], arrcheck.I32)]

    @classmethod
    def empty(cls, num: int):
        return cls(
            xyz=torch.empty([num, 3], dtype=torch.float32),
...
class ColmapSceneSplitChunks:
    ...
    def _get_laplacian_vars(self, images_dir, image_metas):
        with ThreadPoolExecutor() as executor:
            res = []
            for res_item in tqdm.tqdm(executor.map(partial(get_var_of_laplacian, images_dir=images_dir, image_metas=image_metas), image_metas), total=len(image_metas)):
                res.append(res_item)
    ...
    def __call__(self, scene: Scene[BasicFrame]) -> list[Scene[BasicFrame]]:
        ...
        laplacian_vars = self._get_laplacian_vars(meta.colmap_image_root, images_metas)
        ...
        points3d_ordered[colmap_point3d.indice.numpy()] = colmap_point3d.xyz.numpy()
        images_points3d = {}
        ...
        global_bbox = np.stack([cam_centers.min(axis=0)[:2], cam_centers.max(axis=0)[:2]])
        ...
        padd = np.array([chunk_width * self.chunk_size - extent[0], chunk_height * self.chunk_size - extent[1]])

        padd_ref = np.array([self.chunk_size - extent[0] % self.chunk_size, self.chunk_size - extent[1] % self.chunk_size])
    ...
    def make_chunk(self, i: int, j: int, n_width: int, n_height: int, global_bbox: np.ndarray, colmap_point3d: ColmapPoint3d, images_metas: dict, images_points3d: dict, cam_centers: np.ndarray, laplacians_dict: dict):
        # in_path = f"{args.base_dir}/chunk_{i}_{j}"
        # if os.path.exists(in_path):
        print(f"chunk {i}_{j}")
        # corner_min, corner_max = bboxes[i, j, :, 0], bboxes[i, j, :, 1]
        corner_min = global_bbox[0] + np.array([i * self.chunk_size, j * self.chunk_size, 0])
        corner_max = global_bbox[0] + np.array([(i + 1) * self.chunk_size, (j + 1) * self.chunk_size, 1e12])
        # corner_min[2] = -1e12
        # corner_max[2] = 1e12
        ...
        mask_lower = np.all(colmap_point3d.xyz.numpy() < corner_max_for_pts, axis=-1)
        mask_upper = np.all(colmap_point3d.xyz.numpy() > corner_min_for_pts, axis=-1)
        ...
        valid_cam = np.logical_and(np.all(cam_centers[:2] < corner_max, axis=-1), np.all(cam_centers[:2] > corner_min, axis=-1))
