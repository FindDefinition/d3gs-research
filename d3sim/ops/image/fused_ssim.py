...
from cumm.inliner.sympy_codegen import VectorSymOperator, VectorExpr
        ...
        with code.for_(f"int i = 0; i < {num_channels}; ++i"):
            ...
            with code.if_("pixel_valid"):
                ...
                if training:
                    ...
                    auto mu22 = val_yy;
                    auto mu12 = val_xy;
                    // generated by sympy, so it's little bit ugly
                    // check _test_check_ssim_grad in this file for more details
...
def _test_ssim_bwd():
    img1 = torch.rand(8, 3, 960, 1280).to(D3SIM_DEFAULT_DEVICE)
    img2 = torch.rand(8, 3, 960, 1280).to(D3SIM_DEFAULT_DEVICE)
